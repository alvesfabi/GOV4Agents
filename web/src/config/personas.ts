 ...
 * test tenant (the "EntraAgentID Demo" users). Each value can be overridden
 * per tenant via web/.env.local (VITE_SPONSOR_UPN, VITE_APPROVER_UPN,
...
export const personas: { sponsor: Persona; approver: Persona; manager: Persona } = {
  sponsor: {
    displayName: (env.VITE_SPONSOR_NAME as string) || 'EntraAgentID Demo1',
    upn: (env.VITE_SPONSOR_UPN as string) || 'EntraAgentIDDemo1@M365x20582634.onmicrosoft.com',
  },
  approver: {
    displayName: (env.VITE_APPROVER_NAME as string) || 'EntraAgentID Demo2',
    upn: (env.VITE_APPROVER_UPN as string) || 'EntraAgentIDDemo2@M365x20582634.onmicrosoft.com',
  },
  manager: {
    displayName: (env.VITE_MANAGER_NAME as string) || 'EntraAgentID Demo3',
